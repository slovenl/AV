...
#define LOG_TAG "LiveShowMediaCodecVideoDecoder"

LiveShowMediaCodecVideoDecoder::LiveShowMediaCodecVideoDecoder() {
	rtmp_tcurl = NULL;
}

LiveShowMediaCodecVideoDecoder::LiveShowMediaCodecVideoDecoder(JavaVM *g_jvm, jobject obj)
		: MediaCodecVideoDecoder(g_jvm, obj) {
	rtmp_tcurl = NULL;
}
...
void LiveShowMediaCodecVideoDecoder::setRTMPCurl(char* rtmp_tcurl) {
	if (NULL != rtmp_tcurl) {
...
void LiveShowMediaCodecVideoDecoder::initFFMpegContext() {
...
int LiveShowMediaCodecVideoDecoder::openFormatInput(char *videoSourceURI) {
	AVDictionary *options = NULL;
	if (NULL != rtmp_tcurl) {
		LOGI("Media Codec decoder openFormat input rtmp_tcurl is %s", rtmp_tcurl);
 ...
 */
int LiveShowMediaCodecVideoDecoder::initAnalyzeDurationAndProbesize(int* max_analyze_durations, int analyzeDurationSize, int probesize, bool fpsProbeSizeConfigured) {
	//这里注意了 比较坑人
...
int LiveShowMediaCodecVideoDecoder::openFile(DecoderRequestHeader *requestHeader){
	int ret = MediaCodecVideoDecoder::openFile(requestHeader);
	if(ret >= 0){
		...
		while(((videoWidth <= 0 || videoHeight <= 0) && retryTimes > 0)){
			...
			usleep(500 * 1000);
			connectionRetry = 1;
			ret = MediaCodecVideoDecoder::openFile(requestHeader);
			if(ret < 0){
				break;
			}
			retryTimes--;
...
bool LiveShowMediaCodecVideoDecoder::isNeedRetry() {
	this->connectionRetry++;
	return VideoDecoder::isNeedRetry() && (this->connectionRetry <= NET_WORK_STREAM_RETRY_TIME);
}

void LiveShowMediaCodecVideoDecoder::flushVideoFrames(AVPacket packet, int* decodeVideoErrorState) {
