...
void GLrenderS::SurfaceCreate() {
    ...
    //获取统一变量的数量
    // 1.统一变量块中的统一变量
    // 2.着色器代码中声明的默认统一变量块中的统一变量
    // 3.着色器代码中使用的内建同一边变量
    GLint status = 0;
    GLuint index = 0;
    glGetProgramiv(tProgram, GL_ACTIVE_UNIFORMS, &status);

    glVertexAttribPointer(index, 1/*1~4*/, GL_BYTE, GL_FALSE, 0, &status);
//    glGetActiveUniform(tProgram, 0, 4, NULL, &status, GL_F, nullptr);
